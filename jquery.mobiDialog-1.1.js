	...
	$.fn.mobiDialog = function(options){
		...
		return this.each(function(){
			...
			var building = {//构建
				init : function(){
					...
					if((options.type == "warn")&&(options.lock == false)){
						//screen_lock(_self);
						screen_lock(_self);
						$('#screen_lock').css('background','none');
					}else{ 
						screen_lock(_self);
					}
					...
					if($("."+options.type+"_layer",_self)){
						...
						$("#screen_lock>div>div").append(layer_html);		//注入
					}else{ 
						$("#screen_lock>div>div").append(layer_html);
					}
				...
				position : function(){
						...
						if(options.position == "top"){
							$("."+ options.type +"_layer").attr("style","position:absolute; top:0px; left:0px; right:0px; border-radius:0px;");
							$("."+ options.type +"_layer").addClass("top");
						}else if(options.position == "bottom"){
							$("."+ options.type +"_layer").attr("style","position:absolute; bottom:0px; left:0px; right:0px; border-radius:0px;");
							$("."+ options.type +"_layer").addClass("bottom");
						}else{
							$("."+ options.type +"_layer").attr("style","margin:0 auto;width:50%;");
						}
				},
				build_warn : function(){
			...
			var screen_lock = function(con){ 
				var lock_div = "<div id='screen_lock' style='position:fixed;top:0px;left:0px;bottom:0px;z-index:9998;width:100%;height:100%;'>"+
